...
``` shell
vi /etc/nginx/conf.d/example.org.conf
```
``` shell
server {
...
```
查看 ** 域名.conf**，您会发现 certbot 已经进行了修改

Auto update cert
``` shell
crontab -e
6 0 15 * * /usr/bin/certbot renew --quiet
```

``` example.org.conf
proxy_cache_path /var/www/html/cache levels=1:2 keys_zone=my_cache_static:300m max_size=3g inactive=4096m use_temp_path=off;
server
    {
        listen 80;
        #listen [::]:80;
        ...
        access_log  /var/www/html/example.org.log;
    }
